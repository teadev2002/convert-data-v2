...
import { parseHotelData, formatDateField, getNvCall, getDocDateZalo, getUpDateZalo, getDocDateEmail, getUpDateEmail, getReadAiZalo, getCheckAiZalo, getReadAiEmail, getCheckAiEmail } from '../utils/parser.js';
...
export default function Hotel4MailView({ data, onNavigate, onImportData, currentFileName }) {
  ...
  const getMappedJsonData = () => {
    return data.map((item, index) => ({
      ...
      "Ngày tương tác": formatDateField(item["Ngày tương tác"]),
      "Tên nv gọi": getNvCall(item),
      "Emai": item["Emai"] || item["Email liên hệ"] || '',
      "Gửi email": formatDateField(item["Gửi email"]),
      "Nhắc L1": formatDateField(item["Nhắc L1"] || item["Nhắc lần 1 (Email)"]),
      "Nhắc lần 2": formatDateField(item["Nhắc lần 2"] || item["Nhắc lần 2 (Email)"]),
      "Ngày nhận Báo giá, Hợp đồng": getDocDateEmail(item),
      "Ngày up TT NCC": getUpDateEmail(item),
      "Đọc AI": getReadAiEmail(item),
      "Kiểm tra Dữ liệu AI": getCheckAiEmail(item),
      "Zalo": item["Zalo"] || '',
      "Gửi zalo": formatDateField(item["Gửi zalo"]),
      "Nhắc lần 1": formatDateField(item["Nhắc lần 1"] || item["Nhắc lần 1 (Zalo)"]),
      "Nhắc lần 2": formatDateField(item["Nhắc lần 2"] || item["Nhắc lần 2 (Zalo)"]),
      "Ngày nhận Báo giá, Hợp đồng (Zalo)": getDocDateZalo(item),
      "Ngày up TT NCC (Zalo)": getUpDateZalo(item),
      "Đọc AI (Zalo)": getReadAiZalo(item),
      "Kiểm tra Dữ liệu AI (Zalo)": getCheckAiZalo(item),
      "Khác": item["Khác"] || '',
  ...
  const columns = [
    ...
    { title: 'Ngày tương tác', dataIndex: 'interactDate', width: 130, render: (_, item) => formatDateField(item["Ngày tương tác"]) },
    { title: 'Tên nv gọi', dataIndex: 'nvCall', width: 120, render: (_, item) => getNvCall(item) },
    { title: 'Emai', dataIndex: 'emai', width: 130, render: (_, item) => item["Emai"] || item["Email liên hệ"] || '' },
    { title: 'Gửi email', dataIndex: 'sendMail', width: 120, render: (_, item) => formatDateField(item["Gửi email"]) },
    { title: 'Nhắc L1', dataIndex: 'remindL1', width: 120, render: (_, item) => formatDateField(item["Nhắc L1"] || item["Nhắc lần 1 (Email)"]) },
    { title: 'Nhắc lần 2', dataIndex: 'remindL2', width: 120, render: (_, item) => formatDateField(item["Nhắc lần 2"] || item["Nhắc lần 2 (Email)"]) },
    { title: 'Ngày nhận Báo giá, Hợp đồng', dataIndex: 'docDate1', width: 200, render: (_, item) => getDocDateEmail(item) },
    { title: 'Ngày up TT NCC', dataIndex: 'upDate1', width: 150, render: (_, item) => getUpDateEmail(item) },
    { title: 'Đọc AI', dataIndex: 'readAi1', width: 100, render: (_, item) => getReadAiEmail(item) },
    { title: 'Kiểm tra Dữ liệu AI', dataIndex: 'checkAi1', width: 160, render: (_, item) => getCheckAiEmail(item) },
    { title: 'Zalo', dataIndex: 'zalo', width: 100, render: (_, item) => item["Zalo"] || '' },
    { title: 'Gửi zalo', dataIndex: 'sendZalo', width: 100, render: (_, item) => formatDateField(item["Gửi zalo"]) },
    { title: 'Nhắc lần 1', dataIndex: 'remindZalo1', width: 120, render: (_, item) => formatDateField(item["Nhắc lần 1"] || item["Nhắc lần 1 (Zalo)"]) },
    { title: 'Nhắc lần 2', dataIndex: 'remindZalo2', width: 120, render: (_, item) => formatDateField(item["Nhắc lần 2"] || item["Nhắc lần 2 (Zalo)"]) },
    { title: 'Ngày nhận Báo giá, Hợp đồng (Zalo)', dataIndex: 'docDate2', width: 200, render: (_, item) => getDocDateZalo(item) },
    { title: 'Ngày up TT NCC (Zalo)', dataIndex: 'upDate2', width: 150, render: (_, item) => getUpDateZalo(item) },
    { title: 'Đọc AI (Zalo)', dataIndex: 'readAi2', width: 100, render: (_, item) => getReadAiZalo(item) },
    { title: 'Kiểm tra Dữ liệu AI (Zalo)', dataIndex: 'checkAi2', width: 160, render: (_, item) => getCheckAiZalo(item) },
